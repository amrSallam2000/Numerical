...
namespace Numerical.Classes
{
    internal class false_pos
    {
        ...
        double fixed_point(double xl, double xu,double eps)
        {
            ...
            {
                XrOld = Xr;
                Xr = xu - (f.FunctionValue(xu) * (xl - xu) / (f.FunctionValue(xl) - f.FunctionValue(xu)));
                ...
                {
                    print(iter, xl, f.FunctionValue(xl), xu, f.FunctionValue(xu), Xr, f.FunctionValue(Xr), Error.ToString("N3")+" %");
                }
        ...
        public void main(string fun,double xl, double xu, double eps, Label l,DataGridView dv)
        {
            double root;
